...
async def admin_add_feature(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if len(context.args) < 3:
        await update.message.reply_text(
            ...
            "Используйте пробелы для разделения параметров:\n"
            "`/add_feature название заголовок описание [версия] [--test]`\n\n"
            "**Примеры:**\n"
            "`/add_feature уведомления Уведомления Система_уведомлений_о_новых_фичах v1.2 --test`\n"
            "`/add_feature приоритеты Приоритеты Установка_приоритетов_задач v1.0`\n\n"
            "**Параметры:**\n"
            "• `название` - техническое название (без пробелов)\n"
            "• `заголовок` - название для пользователей (без пробелов)\n"
            "• `описание` - описание функции (используйте _ вместо пробелов)\n"
            "• `версия` - номер версии (необязательно)\n"
            "• `--test` - флаг тестовой фичи",
            parse_mode='Markdown'
        )
        return
    ...
    # Собираем описание из всех аргументов до версии или флага --test
    description_parts = []
    version = None
    is_test = False
    
    # Начинаем с 3-го аргумента (индекс 2)
    for i, arg in enumerate(context.args[2:], start=2):
        if arg == "--test":
            is_test = True
        elif arg.startswith("v") and "." in arg:  # Похоже на версию
            version = arg
        else:
            description_parts.append(arg)
    
    # Объединяем части описания и заменяем _ на пробелы
